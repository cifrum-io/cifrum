...
class PortfolioStatisticsTest(unittest.TestCase):
    ...
    def test_compound_annual_growth_rate(self):
        p = yapo.portfolio(assets=[('quandl/BND', 1.)],
                           start_period='2011-1', end_period='2017-2',
                           currency='USD')
        cagr_default = p.compound_annual_growth_rate()
        self.assertTrue(abs(cagr_default - 0.03) < 1e-3)

        cagr_long_time = p.compound_annual_growth_rate(years_ago=20)
        self.assertTrue(abs(cagr_default - cagr_long_time) < 1e-3)

        cagr_one_year = p.compound_annual_growth_rate(years_ago=1)
        self.assertTrue(abs(cagr_one_year - .012) < 1e-3)

        cagr_array = p.compound_annual_growth_rate(years_ago=[None, 20, 1])
