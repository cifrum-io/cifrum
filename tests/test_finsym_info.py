...
from cifrum.common.financial_symbol import FinancialSymbol


def test__returns_sym_info():
    info = lib.information(name='micex/FXRU')
    assert_that(info, instance_of(FinancialSymbol))

    info = lib.information(names=[])
    assert_that(info, instance_of(list))
    assert_that(info, empty())

    info = lib.information(names=['infl/RUB'])
    assert_that(info, instance_of(list))
    assert_that(info, has_length(1))

    info = lib.information(names=['micex/FXRU', 'infl/RUB'])
    assert_that(info, instance_of(list))
    assert_that(info, has_length(2))


def test__micex_stocks_should_have_correct_fields():
    info = lib.information(name='micex/SBER')
...
def test__us_data_source_should_have_correct_fields():
    info = lib.information(name='us/VNQ')
...
def test__currency_usd__should_have_correct_fields():
    info = lib.information(name='cbr/USD')
...
def test__inflation_ru__should_have_correct_fields():
    info = lib.information(name='infl/RUB')
...
def test__top_rates__should_have_correct_fields():
    info = lib.information(name='cbr/TOP_rates')
    assert_that(info, has_property('namespace', 'cbr'))
...
def test__all_data_should_be_available():
    assert_that(lib.information(name='us/MSFT'), not_none())
    assert_that(lib.information(name='micex/FXRU'), not_none())
    assert_that(lib.information(name='micex/FXMM'), not_none())
    assert_that(lib.information(name='index/MCFTR'), not_none())
    assert_that(lib.information(name='index/IMOEX'), not_none())
    assert_that(lib.information(name='index/OKID10'), not_none())
    assert_that(lib.information(name='index/^STOXX50E'), not_none())
    assert_that(lib.information(name='mut_ru/0890-94127385'), not_none())
    assert_that(lib.information(name='cbr/USD'), not_none())
    assert_that(lib.information(name='cbr/EUR'), not_none())
    assert_that(lib.information(name='infl/RUB'), not_none())
    assert_that(lib.information(name='infl/USD'), not_none())
    assert_that(lib.information(name='infl/EUR'), not_none())
    assert_that(lib.information(name='cbr/TOP_rates'), not_none())


def test__return_none_if_no_finsym_is_found():
    not_existing_id = 'micex/MCFTR_doesntexist'
    assert_that(lib.information(name=not_existing_id), none())

    infos = lib.information(names=['infl/RUB', not_existing_id])
...
def test__return_same_infos_count_as_provided():
    ...
    infos = lib.information(names=ids_arr)
    assert len(infos) == len(ids_arr)


def test__be_invariant_in_respect_to_order():
    infos1 = [info.identifier.format() for info in lib.information(names=['infl/RUB', 'infl/EUR'])]
    infos2 = [info.identifier.format() for info in lib.information(names=['infl/EUR', 'infl/RUB'])]
