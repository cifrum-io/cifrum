...
def test__handle_the_incorrect_query_string():
    assert_that(calling(y.search).with_args(query=None), raises(ValueError))
    assert_that(calling(y.search).with_args(query=42), raises(ValueError))
    assert_that(calling(y.search).with_args(query=.42), raises(ValueError))
    assert_that(calling(y.search).with_args(query=[]), raises(ValueError))
    assert_that(y.search(query=''), empty())

    assert_that(calling(y.search).with_args(query='microsoft', top=[]), raises(ValueError))
    assert_that(calling(y.search).with_args(query='microsoft', top=.2), raises(ValueError))
    assert_that(calling(y.search).with_args(query='microsoft', top='a'), raises(ValueError))
...
def test__search_exact_ticker(query, expect_item):
    def __search(query: str):
        search_results = y.search(query=query)
...
def test__search_custom_query():
    ...
    rs = y.search(query='microsoft', top=30)
    for r in rs:
